	...
	public class SingleTableEntityPersister : AbstractEntityPersister, IQueryable
	{
			...
			: base(persistentClass, cache, factory)
		{
			...
			foreach (Join join in persistentClass.SubclassJoinClosureIterator)
			{
				...
				{
					int curTableIndex = joinKeyColumns.Count - 1;
					//there should only ever be one key
					List<string> toKeyCols = new List<string>(join.RefIdProperty.ColumnSpan);
		...
		protected override object GetJoinTableId(int table, object obj, EntityMode entityMode)
		{
			...
			int refIdColumn = GetRefIdColumnOfTable(table);
			if (refIdColumn == -1)
				return null;
		...
		private object GetJoinTableId(int table, int index, object[] fields)
		{
			if (index != -1)
				return fields[index];
		...
		{
			if (tableIdPropertyNumbers.ContainsKey(table))
				return tableIdPropertyNumbers[table];
...
- map the its subclasses.";
		...
		{
			if (joinToKeyColumns.ContainsKey(j))
				return joinToKeyColumns[j];
